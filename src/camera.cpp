...
void Camera::Animate(float t) 
{
    if (freemode)
    {
        ...
        glm::vec3 v =
                velocityOrientation == glm::vec3(0, 0, 0) ?
                    glm::vec3(0, 0, 0) :
                    movingSpeed*glm::normalize(velocityOrientation);
    ...
    else if (!freemode)
    {
        ...
        if (a == true)
        {
            ...
            if (!arrived) 
            {
                //The direction need to be rotated too, 
                //because the rotation around the center could slow the turning
                lookAtPoint = glm::rotate(lookAtPoint,
                                          -rotationSpeed*t,
                                          glm::vec3(0, 1, 0));
            }
        ...
        if (d == true)
        {
            ...
            if (!arrived)
            {
                lookPoint = glm::rotate(lookPoint,
                                        rotationSpeed*t,
                                        glm::vec3(0, 1, 0));
            }
            position = glm::rotate(position,
        ...
        {
            //The camera is looking toward the center point:
            orientation = glm::normalize(targetPoint - position); 
        }
        else
        {
            glm::vec3 rotationAxis = glm::cross(orientation,
                                                targetPoint - position);
            
            // The axis vector could be (0,0,0), even if it's rare:
            if (rotationAxis.x == 0.0f && 
                rotationAxis.y == 0.0f && 
                rotationAxis.z == 0.0f)
            {
                //The (0,0,0) axis indicates, that the angle is 0:
                arrived = true; 
                orientation = glm::normalize(targetPoint - position);
            }
            else
            {
                glm::vec3 nextorientation = glm::rotate(orientation,
                                                        lookAtpointSpeed*t,
                                                        rotationAxis);
                glm::vec3 lookAtVector = targetPoint - position;

                //If it "outswings" the target point, 
                //the new rotationAxis will be pointing 
                //at the opposite direction
                if (glm::dot(rotationAxis,
                             glm::cross(nextorientation, lookAtVector)) <= 0.0f)
                {
                    arrived = true;
...
void Camera::setOrientation(float x,float y,float z)
{
    if (!freemode) 
    {
        ...
        position = targetPoint +
                getDistance() *
                glm::normalize(-orientationNotNormalized);
...
void Camera::rotate(float dx, float dy)
{
    ...
    else
    {
        angleY = angleY + dy;
        orientation = glm::normalize(
                    glm::rotate(orientation, glm::radians(-dy), right));
    }
}
...
void Camera::setFreeMode(bool value) 
{
    ...
    {
        //The program doesn't allow vertical angle 
        //greater than 90 or lesser than 0.
        //At the change from not free to free mode, 
        //it have to calculate the actual vertical angle
        angleY = glm::degrees(
                    glm::angle(glm::normalize(orientation), glm::vec3(0,1,0)));
    }
    freemode = value;
}
