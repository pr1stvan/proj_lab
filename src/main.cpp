...
void animateModel() 
{
    double timeSecs = glfwGetTime();  // time since the start of the program
...
// Callback function called when the 'AutoRotate' 
// variable value of the tweak bar has changed
...
{
    model.lightSource1.position = (*(const float*)value) *
        glm::normalize(model.lightSource1.position);
}

void TW_CALL lightSource2SetDistance(const void *value, void *clientData)
{
    model.lightSource2.position = (*(const float*)value) *
        glm::normalize(model.lightSource2.position);
}

void TW_CALL lightSource1SetDirection(const void *value, void *clientData)
{
    model.lightSource1.position = 
        -glm::length(model.lightSource1.position) *
            glm::vec3(((const float*)value)[0], 
            ((const float*)value)[1], 
            ((const float*)value)[2]);
}
...
{
    model.lightSource2.position = -glm::length(
            model.lightSource2.position) * glm::vec3(((const float*)value)[0], 
            ((const float*)value)[1], 
            ((const float*)value)[2]);
}
...
void initializeAntTweakBar() 
{
    ...
    TwAddVarCB(bar,
               ...
               "opened=false "
               "help='Change the camera direction in target mode.' ");
    TwAddVarRW(bar,
               "Zoom",
               TW_TYPE_FLOAT,
               &camera->viewAngle,
               ...
               "keyIncr=q "
               "keyDecr=e "
               "help='Adjust the FOV of the camera.' ");
    TwAddVarCB(bar,
               "cameraDistance",
               TW_TYPE_FLOAT,
               setDistance,
               ...
               "min=1.0 "
               "step=0.3 "
               "help = 'Adjust the distance of the camera in target mode' ");
    TwAddVarCB(bar,
               "AutoRotate",
               TW_TYPE_BOOL8,
               setFreemode,
    ...
    TwAddVarRW(bar,
               ...
               "group='display' "
               "label='show 3D grid' "
               "help='Draw a cube around the 3D grid' ");
    TwAddVarRW(bar,
               "frameSpeed",
               TW_TYPE_FLOAT,
               &particleSystem->maxFps,
               "group='display' "
               "label='max fps' "
               "min=1.0 "
               "max=1000.0 "
               "step=0.2 "
               "help='Adjust the number of prt file frames in one second' ");
    TwAddVarRW(bar,
               "prtFolder",
               ...
               "label='prt folder' "
               "help='This is the folder of the prt files.' ");
    TwAddVarRW(bar,
               "loadBefore",
               TW_TYPE_BOOL8,
               &particleSystem->loadMode,
               "group='file parameters' "
               "label='load before' "

               "help='true: Loading the particles "
               "from all files before rendering\nfalse: "
               "Loading the files while rendering' ");
    TwAddVarRW(bar,
               "onlyFileParticles",
               TW_TYPE_BOOL8,
               &particleSystem->onlyFileParticles,
               "group='file parameters' "
               "label='no generation'  "
               "help='If its true, the program doesnt generate particles' ");
    TwAddVarRW(bar,
               "generatedParticleFile",
               TW_TYPE_INT32,
               &particleSystem->generatedParticleFile,
               "group='file parameters' "
               "label='generation base' "

               "help='The number of the frame that contains "
               "the control particles for generating.' ");
    TwAddVarRW(bar,
               "cubeLength",
               TW_TYPE_FLOAT,
               &particleSystem->cubeLength,
               "group='3D array' "
               "min=0.0001 "
               "label='3D grid width' "
               "step=0.1 "

               "help='Adjust the width of the space, "
               "where the 3D grid is applied' ");
    TwAddVarRW(bar,
               "splitting",
               TW_TYPE_INT32,
               &particleSystem->velocityArrayParts,
               "group='3D array' "
               "min=1 "
               "max=100 "
               "step=1 "
               "label='grid splitting' "
               "help='Adjust the number of vectors in one 3d Array row' ");
    TwAddVarRW(bar,
               "genPerParticle",
               TW_TYPE_INT32,
               &particleSystem->genPerParticle,
               "group='generated particles' "
               "label='num of particles' "
               "min=1 "
               "step=1 "
               "max=100 "

               "help='Adjust the number of particles, "
               "that generated for each file particle' ");
    TwAddVarRW(bar,
               "atwR",
               TW_TYPE_FLOAT,
               &particleSystem->atwR,
               ...
               "min=0.1 "
               "label='max distance' "
               "help='adjust the max distance from the original particle'");
    TwAddVarRW(bar,
               "particleColor",
               TW_TYPE_COLOR3F,
               &particleSystem->generatedParticleColor,
               " group='generated particles' "
               "label='particle color' "
               "help='Change the color of the particles in load time' ");
    TwAddVarRW(bar,
               "ignoreNullVelocity",
               TW_TYPE_BOOL8,
               &particleSystem->ignoreNullVelocity,
               "group='Corrections' "
               "label='ignore null velocity' "
               "help='ignore the 0 elements when average the velocity'");
    TwAddVarRW(bar,
               "followControllPoints",
               TW_TYPE_BOOL8,
               &particleSystem->followControllPoints,
               "group='Corrections' "
               "label='follow controllpoints' "

               "help='true: Ha az eredeti pont tul messze van, "
               "a generalt pont kovetni fogja.'");
    TwAddVarRW(bar,
               "distanceMultiplier",
               TW_TYPE_FLOAT,
               &particleSystem->distanceMultiplier,
               "group='Corrections' "
               "label='follow speed' "
               "max=1 min=0 help='A kovetes sebessege'");
    TwAddVarRW(bar,
               "randomR",
               TW_TYPE_FLOAT,
               &particleSystem->randomR,
               "group='Corrections' "
               "label='random r' "
               "min=0 "

               "help='Ha nem 0, nem fogja teljesen "
               "az eredeti pontot kovetni'");
    TwAddVarRW(bar,
               "triggerDistance",
               TW_TYPE_FLOAT,
               &particleSystem->triggerDistance,
               "group='Corrections' "
               "label='trigger distance' "
               "min=0 "

               "help='A tavolsag, "
               "ami folott korrigalja a pont poziciojat'");
    TwBar *lightsBar = TwNewBar("Lights");
    TwDefine(" Lights label='Light settings' "
             "position='10 520' "
             "size='220 320' "
             "help='Felulet a fenyforrasok tulajdonsagainak beallitasara.' ");
    
    TwAddVarCB(lightsBar,
               "useLights",
               TW_TYPE_BOOL8,
               setUseLights,
               getUseLights,
               ...
               "group='' "
               "label = 'use lights' "
               "key=l help = 'Change to free mode.' ");
    TwAddVarRW(lightsBar,
               "particleSmoothness",
               TW_TYPE_FLOAT,
               &view.particleSmoothness,
               "group='' "
               "label='particle smoothness' "
               "min=0.1  "
               "max=10.0 "
               "step=0.1 "
               "help='Smootness of the surface of the particles'");

    TwAddVarCB(lightsBar,
               "lightSource1Direction",
               TW_TYPE_DIR3F,
               lightSource1SetDirection,
               lightSource1GetDirection,
               NULL,
               "group='light source 1' "
               "label='direction' "
               "opened=false "
               "help='Change the direction of the first light source' ");
    TwAddVarCB(lightsBar,
               "lightSource1Distance",
               TW_TYPE_FLOAT,
               lightSource1SetDistance,
               lightSource1GetDistance,
               NULL,
               "group='light source 1' "
               "label = 'distance' "
               "min=0.3 "
               ...
               "help = 'Set the distance of the "
               "origin for the first light source' ");
    TwAddVarRW(lightsBar,
               "lightSource1Color",
               TW_TYPE_COLOR3F,
               &model.lightSource1.color,
               " group='light source 1' "
               "label='color' "

               "help='Color of the first light source' ");
    TwAddVarRW(lightsBar,
               "lightSource1LightPower",
               TW_TYPE_FLOAT,
               ...
               "help='Adjust the power of the light' ");
    TwAddVarRW(lightsBar,
               "lightSource1SpecularPower",
               TW_TYPE_FLOAT,
               ...
               "help='Adjust the power of the light' ");
    TwAddVarRW(lightsBar,
               "lightSource1DrawSphere",
               TW_TYPE_BOOL8,
               &view.drawSphereForLightSource1,
               "group='light source 1' "
               "label='draw sphere' "

               "help='Draw sphere to the position of the first light source'");

    TwAddVarCB(lightsBar,
               "lightSource2Direction",
               TW_TYPE_DIR3F,
               lightSource2SetDirection,
               lightSource2GetDirection,
               NULL,
               "group='light source 2' "
               "label='direction' "
               "opened=false "

               "help='Change the direction of the second light source' ");
    TwAddVarCB(lightsBar,
               "lightSource2Distance",
               TW_TYPE_FLOAT,
               lightSource2SetDistance,
               lightSource2GetDistance,
               NULL,
               "group='light source 2' "
               "label = 'distance' "
               "min=0.3 "
               ...
               "help = 'Set the distance of the origin"
               " for the second light source' ");
    TwAddVarRW(lightsBar,
               "lightSource2Color",
               TW_TYPE_COLOR3F,
               &model.lightSource2.color,
               " group='light source 2' "
               "label='color' "
               "help='Color of the first light source' ");
    TwAddVarRW(lightsBar,
               "lightSource2LightPower",
               TW_TYPE_FLOAT,
    ...
    TwAddVarRW(lightsBar,
               ...
               "step=10 "
               "help='Adjust the power of the light' ");
    TwAddVarRW(lightsBar, "lightSource2DrawSphere",
               TW_TYPE_BOOL8,
               &view.drawSphereForLightSource2,
               "group='light source 2' "
               "label='draw sphere' "
               "help='Draw sphere to the position of the second light source'");
