...
bool LoadBeforeFrameLoader::getFrame(unsigned int index, 
...
void LoadBeforeFrameLoader::calculateVelocities()
{
    ...
    for (unsigned int i = 0; i <frames.size() -1; i++)
    {
        ...
        for (unsigned int j = 0;
             j<particles.size() && j<particlesNextFile.size(); j++)
        {
...
bool LoadBeforeFrameLoader::loadFiles(const std::vector<std::string>& files)
{
    ...
    if(!ParticleArrayLoader::getParticleCountFromFirstFile(
                std::string(fileNames[0]), particleCountPerFrame))
    {
        return false;
    }
    ...
    {
        std::vector<Particle> particleArray;
        bool succes = ParticleArrayLoader::loadParticlesFromFile(
                 particleArray, fileNames[i]);
