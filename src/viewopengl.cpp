...
void ViewOpenGL::loadCube() 
{
	...
	// vertex coordinates
	glBindBuffer(GL_ARRAY_BUFFER, cubePointPositionBuffer); // make it active, it is an array
...
void ViewOpenGL::initialize(Model *model, IShaderProgramProvider &shaderProgramProvider)
{
    simpleShaderProgram = shaderProgramProvider.getSimpleShaderProgram();
    particleDrawShaderProgram = shaderProgramProvider.getParticleDrawShaderProgram();
	...
	glGenBuffers(1, &particleNormalBuffer);	// Generate a vertex buffer object
	glGenBuffers(1, &particleIndexBuffer);	// Generate a vertex buffer object
	glGenBuffers(1, &loadedParticleBuffer);	// Generate a vertex buffer object
	glGenBuffers(1, &generatedParticleBuffer);	// Generate a vertex buffer object
	glGenBuffers(1, &cubePointPositionBuffer);	// Generate a vertex buffer object
	glGenBuffers(1, &lightSourceVertexBuffer); // Generate a vertex buffer object
	...
	// vertex coordinates
	glBindBuffer(GL_ARRAY_BUFFER, particlePointBuffer); // make it active, it is an array
	glBufferData(GL_ARRAY_BUFFER, sphere.vertices.size() * sizeof(float), &sphere.vertices[0], GL_STATIC_DRAW);

	// vertex normals
	glBindBuffer(GL_ARRAY_BUFFER, particleNormalBuffer); // make it active, it is an array
	glBufferData(GL_ARRAY_BUFFER, sphere.normals.size() * sizeof(float), &sphere.normals[0], GL_STATIC_DRAW);

	//indexes for coordinates and normals
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, particleIndexBuffer);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sphere.indexes.size() * sizeof(unsigned int), &sphere.indexes[0], GL_STATIC_DRAW);

	//Light source spheres
	sphere = createSphereOpenGl(20, 20, 1.0f);
	lightSourceIndexCount = sphere.indexes.size();
	// vertex coordinates
	glBindBuffer(GL_ARRAY_BUFFER, lightSourceVertexBuffer); // make it active, it is an array
	glBufferData(GL_ARRAY_BUFFER, sphere.vertices.size() * sizeof(float), &sphere.vertices[0], GL_STATIC_DRAW);
...
{
	glBindBuffer(GL_ARRAY_BUFFER, loadedParticleBuffer); // make it active, it is an array
	// Initialize with empty (NULL) buffer : it will be updated later, each frame.
	glBufferData(GL_ARRAY_BUFFER,      // copy to the GPU
		model->getOriginalParticleCount() * sizeof(Particle), // number of the vbo in bytes
		NULL,	// pointer to the data
		GL_STREAM_DRAW);

	// Initialize with empty (NULL) buffer : it will be updated later, each frame.
	glBindBuffer(GL_ARRAY_BUFFER, generatedParticleBuffer); // make it active, it is an array
	glBufferData(GL_ARRAY_BUFFER,      // copy to the GPU
		model->getGeneratedParticleCount() * sizeof(Particle), // number of the vbo in bytes
		NULL,	// pointer to the data
		GL_STREAM_DRAW);
}
...
void ViewOpenGL::renderParticlesOpenGL() 
{
	...
	glm::mat4 V = getCameraV(model->getCameraPosition(), model->getCameraOrientation());
	...
	if (useLights) 
	{
		...
		glUniform1f(particleShader_Light1Power_ID, model->lightSource1.lightPower);
		glUniform1f(particleShader_Light2Power_ID, model->lightSource2.lightPower);
		glUniform1f(particleShader_Light1SpecularPower_ID, model->lightSource1.specularPower);
		glUniform1f(particleShader_Light2SpecularPower_ID, model->lightSource2.specularPower);
	}
	else 
	{
	...
	glUniform3fv(particleShader_Light1Position_worldspace_ID, 1, &model->lightSource1.position[0]);
	glUniform3fv(particleShader_Light1Color_ID, 1, &model->lightSource1.color[0]);
	glUniform3fv(particleShader_Light2Position_worldspace_ID, 1, &model->lightSource2.position[0]);
	glUniform3fv(particleShader_Light2Color_ID, 1, &model->lightSource2.color[0]);
	...
	const std::vector<Particle>& originalParticles = model->getOriginalParticles();
	const std::vector<Particle> & generatedParticles = model->getGeneratedParticles();
	
	// center coordinates and colors
	glBindBuffer(GL_ARRAY_BUFFER, loadedParticleBuffer); // make it active, it is an array
	//Buffer orphaning from opengl tutorial. Increases speed at about 0.7 fps 
	glBufferData(GL_ARRAY_BUFFER,      // copy to the GPU
		originalParticles.size() * sizeof(Particle), // number of the vbo in bytes
		NULL,
		GL_STREAM_DRAW);	   // copy to that part of the memory which is not modified
	glBufferSubData(GL_ARRAY_BUFFER, 0, originalParticles.size() * sizeof(Particle), &originalParticles[0]);

	glBindBuffer(GL_ARRAY_BUFFER, generatedParticleBuffer); // make it active, it is an array
	//Buffer orphaning from opengl tutorial. Increases speed at about 0.7 fps
	glBufferData(GL_ARRAY_BUFFER,      // copy to the GPU
		generatedParticles.size() * sizeof(Particle), // number of the vbo in bytes
		NULL,
		GL_STREAM_DRAW);	   // copy to that part of the memory which is not modified
	glBufferSubData(GL_ARRAY_BUFFER, 0, generatedParticles.size() * sizeof(Particle), &generatedParticles[0]);
	...
	glDrawElementsInstanced(GL_TRIANGLES, sphereIndexCount, GL_UNSIGNED_INT, 0, originalParticles.size());

	bindParticleBuffer(generatedParticleBuffer);
	glDrawElementsInstanced(GL_TRIANGLES, sphereIndexCount, GL_UNSIGNED_INT, 0, generatedParticles.size());
...
void ViewOpenGL::bindParticleBuffer(unsigned int bufferID)
{
	...
	// Map Attribute Array 1 to the current bound vertex buffer (particleColorBuffer)
	...
	glVertexAttribPointer(2, // Attribute Array 2
		...
		GL_FALSE, //normalize? 
		sizeof(Particle), //stride: A particle difference between positions
		(GLvoid*)(sizeof(glm::vec3)));// offset: the second element is a the color(pos,color,v) 
}
...
void ViewOpenGL::renderLightSource(const LightSource& lightSource) 
{
	...
	M = glm::translate(lightSource.position); //Move the lightSource to its position
	M = glm::scale(M,glm::vec3(lightSourceSphereRadius / 1.5f, lightSourceSphereRadius / 1.5f, lightSourceSphereRadius / 1.5f)); // model matrix
	...
	//Vertexes
	glBindBuffer(GL_ARRAY_BUFFER, lightSourceVertexBuffer); // make it active, it is an array
	glEnableVertexAttribArray(0); // Map Attribute Array 0 to the current bound vertex buffer (vbo[0])
	// Data organization of Attribute Array 0
	...
	glDrawElements(GL_TRIANGLES,lightSourceIndexCount,GL_UNSIGNED_INT, (void*)0);

	M = glm::translate(lightSource.position); //Move the lightSource to its position
	M = glm::scale(M, glm::vec3(lightSourceSphereRadius, lightSourceSphereRadius, lightSourceSphereRadius)); // model matrix
	MVPTransform = P* V *M;
...
void ViewOpenGL::renderCube() 
{
	...
	glm::mat4 M = glm::scale(glm::vec3(model->getCubeLength(), model->getCubeLength(), model->getCubeLength())); // model matrix
	glm::mat4 MVPTransform = P*V*M;
...
glm::mat4 ViewOpenGL::getCameraV(glm::vec3 pos, glm::vec3 orientation) // view matrix: translates the center to the origin
{ 
	return glm::lookAt(pos, pos + orientation, glm::vec3(0, 1, 0));
}

glm::mat4 ViewOpenGL::getCameraP(float viewAngle) // projection matrix: scales it to be a square of edge length 2
{ 
	glm::mat4 projection = glm::perspective(glm::radians(viewAngle), (float)windowWidth / (float)windowHeight, 0.1f, 1000.0f);
	return projection;
}
